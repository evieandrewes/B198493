...
To begin, let's look at the number of prescriptions of common hypertension medications, like doxazosin, amlodipine, bisoprolol and atenolol [@hypertension], in a month for each health board in Scotland pre-pandemic (August 2019) and post-pandemic (August 2022).
...
From this table, you can see the number of prescriptions in a month for hypertension has increased by **600,000** from August 2019 to August 2022 in NHS Lothian and Greater Glasgow and Clyde alone. This indicates a huge increase in hypertension prescriptions over the COVID-19 pandemic in these areas. 

However, it is also known these areas are very well populated within Scotland, so it is unknown how *proportional* this increase is to the general population. Let's load in some data about population size per health board, and create a spatial plot to visualize this:
...
ggplot(data = combined_spatial_data) +
  ...
  facet_wrap(~ year) +
  theme_void() +
  labs(title = "Hypertension Prescriptions per Capita per Month", subtitle = "Health Boards in Scotland (August 2019 & August 2022)") +
